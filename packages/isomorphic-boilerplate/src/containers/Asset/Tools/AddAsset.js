...
export default function () {
  ...
  const { assetName,assetId } = useSelector((state) => state.Assets);
  const [strName,setStrName]=React.useState('New Tool #A');
  const [strCode,setStrCode]=React.useState('A');
  const gData = [];
  const [intCategoryID,setIntCategoryID]=React.useState(12);
  ...
  const [locatedFlag,setLocatedFlag]=React.useState('');
  ...
  return (
    <LayoutContentWrapper>
      ...
      <TableTabsStyle className="isoLayoutContentAsset">
        <h4 style={{ marginBottom: "15px" }}>Tools:</h4>
