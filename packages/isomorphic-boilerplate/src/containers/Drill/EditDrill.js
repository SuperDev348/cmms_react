...
const { add,updateData,getById} = drillAction;
...
export default function (props) {
  ...
  const { drill, isDelete} = useSelector((state) => state.Drill);
  ...
  const [aGeneralData, setAGeneralData]=React.useState(null);
  const [aActionData, setAActionData]=React.useState(null);
  ...
  React.useEffect(() => {
    if(Object.keys(drill).length !== 0) {
      if(drill._id != id)
        return;
      setStrStatus(drill.strStatus);
      setDrillCategoryName(drill.strCategory);
      setDrillTypeName(drill.strType);
      setStrDrillTitle(drill.strTitle);
      setIntEstimatedTime(drill.intEstimatedTime);
      setDueDate(drill.aDueDate);
      setStrAssigneeUser(drill.strGeneralAssignee);
      setStrDrillDescription(drill.strGeneralDescription);
      setAssignCompletionDate(drill.aActionAssignCompletionDate);
      setStrNarrative(drill.strActionNarrative);
      setStrHowImproveFuture(drill.strActionNarrativeFuture);
      let tmp = {};
      tmp.strAssignee = drill.strGeneralAssignee;
      tmp.strDescription = drill.strGeneralDescription;
      setAGeneralData(tmp);

      tmp = {};
      tmp.aCompletionDate = drill.aActionAssignCompletionDate;
      tmp.strNarrative = drill.strActionNarrative;
      tmp.strImproveFuture = drill.strActionNarrativeFuture
      setAActionData(tmp);
    }
    // console.log(drill);
  }, [drill])
